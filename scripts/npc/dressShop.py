...
categories = [
    ...
    ["Capes",                20000, 0],
    ["Weapons",              20000, 0],
    ["Face Accessories",     20000, 0],
    ["Eye Accessories",      20000, 0],
    ["Rings",                20000, 0]
]

damageSkinGachaCost = 50000
damageSkinItems = [
    ...
    2438282, # Returned Legend Damage Skin
    2436553, # Returned Legend Damage Skin
    2437697, # Returned Legend Damage Skin
...
def prompt_main():
    text = "#eYou currently have #b" + str(sm.getNX()) +" NX#n\r\n"
    count = 0
    for category in categories:
        if count == 4:
            ...
            text += "\n#b#L" + repr(count) + "#" + str(category[0]) + "#l\r\n"
        count += 1
    text += "#b#L100#Damage Skin Gacha#l"
    return text
...
def prompt_options(item_id):
    ...
    text = (
        ...
        "#L1#Buy for #b{1}#k NX#l\r\n"
        # "#L2#Rent for #b{2}#k NX#l\r\n\r\n"
...
def message_done(item_id, cost, rent):
    ...
    text = (
        ...
        "#b{2}#k NX will be deducted from your account.\r\n\r\n"
...
while flag_main == 1:
    ...
    if selection_main == 100:
        if sm.sendAskYesNo("#bRequired {0} NX#k\r\n\r\nOkay?".format(damageSkinGachaCost)):
            item = damageSkinItems[int(random.uniform(0, len(damageSkinItems)))]
            sm.deductNX(damageSkinGachaCost)
            sm.addDamageSkin(item)
            sm.sendSayOkay("Reward: #v{0}# #z{1}#".format(item, item))
        ...
        while flag_browse == 1:
            ...
            if selection_equips == 9998: # previous
                ...
                if selection_options == 0: # exit - return to list
                    ...
                    if sm.getNX() < cost:
                        sm.sendNext("You do not have enough NX!")
                    else:
